            ...
            if filename.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.gif', '.tiff')):
                ...
                for cluster in new_clusters:
                    ...
                    for existing_cluster, existing_name in list(self.clusters.items()):
                        if self.iou(cluster, existing_cluster) > 0.4:
                            print('replacing')
                            replace = True
...
class BlackjackPlayer:
    ...
    def play_round(self):
        # check if continue is on screen
        if self.click_image('actions/continue.png'):
            print('Continued')
            return
    ...
    def save_player_dealer_images(self, game_image_path, split_percentage=0.4):
        ...
        top_part = img.crop((0, 0, width, split_point//2))
    ...
    def update_splitted_hands(self, player_image_path):
        ...
        width, height = img.size
        split_point = int(width * .5)

        side_cut_off = 300

        left_part = img.crop((side_cut_off, 0, split_point, height))
        right_part = img.crop((split_point, 0, width - side_cut_off, height))
    ...
    def get_active_deck(self):
        image = cv2.imread(self.player_img_path)


    def play_split(self, dealer_hand, right=False):
        # its starts on the left side
        # while right side has 2 cards
        def update_brightness():
            time.sleep(2) # makes sure last round finishes before updating
            self.update_game_images()
            self.update_splitted_hands(self.player_img_path)

            left_img = cv2.imread('game_images/left_hand.png')
            right_img = cv2.imread('game_images/right_hand.png')

            left_gray = cv2.cvtColor(left_img, cv2.COLOR_BGR2GRAY)
            right_gray = cv2.cvtColor(right_img, cv2.COLOR_BGR2GRAY)

            # Calculate the average brightness for each ROI
            left_brightness = np.mean(left_gray)
            right_brightness = np.mean(right_gray)
            print('left_brightness:', left_brightness)
            print('right_brightness:', right_brightness)
            return left_brightness, right_brightness

        left_brightness, right_brightness = update_brightness()

        last_hand = []
        while left_brightness + 5 > right_brightness:
            left_brightness, right_brightness = update_brightness()
            print('left deck is active')
            hand = self.card_checker.get_cards('game_images/left_hand.png', "cards")
            print(hand)
            while len(hand) < 2:
                self.update_game_images()
                self.update_splitted_hands(self.player_img_path)
        ...
        left_brightness, right_brightness = update_brightness()
        last_hand = []
        while right_brightness > left_brightness:
            left_brightness, right_brightness = update_brightness()
            print('right deck is active')
            hand = self.card_checker.get_cards('game_images/right_hand.png', "cards")
            print(hand)
            if len(hand) < len(last_hand):
                break
            move = self.determine_move(hand, dealer_hand, can_split=False)
